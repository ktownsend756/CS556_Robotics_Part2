...
//TODO: Put code under here
  ...
  for(uint8_t i = 0; i < _num_particles; i++){
    ...
    _particle_list[i].x += dx_noise * cos(_particle_list[i].angle) - dy_noise * sin(_particle_list[i].angle);
    _particle_list[i].y += dx_noise * sin(_particle_list[i].angle) + dy_noise * cos(_particle_list[i].angle);
